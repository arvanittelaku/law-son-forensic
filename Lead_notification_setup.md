...
## This project (Lawson Forensic)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `Lawson Forensic` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL` is set) | `lawsonforensic.com` |
| Contact webhook | **`/api/submit-lead`** on contact form submit |
| Google Sheets | Not used for contact (webhook only) |
