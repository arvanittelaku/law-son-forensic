...
// ─── Env helpers (common .env mistakes: wrapping quotes, pasted full Sheet URL) ─
...
export function normalizeSpreadsheetId(raw: string | undefined): string | undefined {
...
export const DEFAULT_SHEET_TAB_NAME = "Lawson Forensic";
...
function appendRangeForTab(sheetName: string, columns = "A:A"): string {
  const name = sheetName || DEFAULT_SHEET_TAB_NAME;
  if (/^[A-Za-z0-9_]+$/.test(name)) return `${name}!${columns}`;
  return `'${name.replace(/'/g, "''")}'!${columns}`;
}
...
export async function appendRow(
  ...
  const spreadsheetId = normalizeSpreadsheetId(target?.spreadsheetId || process.env.GOOGLE_SHEET_ID);
